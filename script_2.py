...
def main():
    ...
    for room in rooms:
        print(f"\n--- {room} Details ---")
        perimeter = get_float_input(f"Enter the perimeter of the {room} in meters (default 10m): ", 10)
        height = get_float_input(f"Enter the height of the {room} in meters (default 3m): ", 3)
        
        num_windows = get_int_input(f"Enter the number of windows in the {room} (default 1): ", 1)
        window_areas = []
        for i in range(num_windows):
            w_length = get_float_input(f"Enter the length of window {i+1} in meters (default 1m): ", 1)
            w_height = get_float_input(f"Enter the height of window {i+1} in meters (default 1.2m): ", 1.2)
            window_areas.append(w_length * w_height)
        
        num_doors = get_int_input(f"Enter the number of doors in the {room} (default 1): ", 1)
        door_areas = []
        for i in range(num_doors):
            d_length = get_float_input(f"Enter the length of door {i+1} in meters (default 0.9m): ", 0.9)
