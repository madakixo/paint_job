...
class PaintCalculator:
    ...
    def calculate_paint_requirement(self, room_name, perimeter, height, window_areas, door_areas, paint_type):
        ...
        coverage_per_100m2 = self.coverage_rates.get(paint_type, None)
    ...
    def run_calculator(self):
        ...
        for room in rooms:
            ...
            perimeter = self.get_float_input(f"Enter the perimeter of the {room} in meters (default {('15.3' if room == 'Parlour' else '12.9' if room == 'Bedroom' else '6' if room == 'Toilet' else '6.6')}m): ", 
                                   15.3 if room == 'Parlour' else 12.9 if room == 'Bedroom' else 6 if room == 'Toilet' else 6.6)
            height = self.get_float_input(f"Enter the height of the {room} in meters (default 3m): ", 3)
            
            num_windows = self.get_int_input(f"Enter the number of windows in the {room} (default {('2' if room == 'Parlour' else '1' if room == 'Bedroom' else '1' if room == 'Toilet' else '1')}): ", 
                                   2 if room == 'Parlour' else 1)
            window_areas = []
            for i in range(num_windows):
                w_length = self.get_float_input(f"Enter the length of window {i+1} in meters (default {('1' if room == 'Parlour' or room == 'Bedroom' else '0.6')}m): ", 
                                              1 if room == 'Parlour' or room == 'Bedroom' else 0.6)
                w_height = self.get_float_input(f"Enter the height of window {i+1} in meters (default {('1.2' if room == 'Parlour' or room == 'Bedroom' else '0.6')}m): ", 
                                              1.2 if room == 'Parlour' or room == 'Bedroom' else 0.6)
                window_areas.append(w_length * w_height)
            
            num_doors = self.get_int_input(f"Enter the number of doors in the {room} (default {('2' if room == 'Parlour' or room == 'Bedroom' else '1')}): ", 
                                 2 if room == 'Parlour' or room == 'Bedroom' else 1)
            door_areas = []
            for i in range(num_doors):
                d_length = self.get_float_input(f"Enter the length of door {i+1} in meters (default {('0.9' if room == 'Parlour' or room == 'Bedroom' else '0.75')}m): ", 
                                              0.9 if room == 'Parlour' or room == 'Bedroom' else 0.75)
                d_height = self.get_float_input(f"Enter the height of door {i+1} in meters (default 2.1m): ", 2.1)
